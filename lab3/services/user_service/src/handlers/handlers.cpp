...
UserRegisterHandler::UserRegisterHandler(const components::ComponentConfig& config,
...
std::string UserRegisterHandler::HandleRequest(server::http::HttpRequest& request,
  ...
  auto created = store_.Create(in.login, auth::HashPassword(in.password), in.first_name, in.last_name);
  if (!created) {
...
std::string UserLoginHandler::HandleRequest(server::http::HttpRequest& request,
  ...
  auto u = store_.FindByLogin(in.login);
...
UserByLoginHandler::UserByLoginHandler(const components::ComponentConfig& config,
                                       const components::ComponentContext& context)
    : HttpHandlerBase(config, context),
      store_(context.FindComponent<components::Postgres>("postgres-db").GetCluster()) {}
...
std::string UserByLoginHandler::HandleRequest(server::http::HttpRequest& request,
  ...
  auto u = store_.FindByLogin(login);
  if (!u) {
...
UserSearchHandler::UserSearchHandler(const components::ComponentConfig& config,
                                     const components::ComponentContext& context)
    : HttpHandlerBase(config, context),
      store_(context.FindComponent<components::Postgres>("postgres-db").GetCluster()) {}
...
std::string UserSearchHandler::HandleRequest(server::http::HttpRequest& request,
  ...
  const auto rows = store_.SearchByNameMask(fn, ln);
  return UsersArrayJson(rows);
}

UserInternalGetHandler::UserInternalGetHandler(const components::ComponentConfig& config,
                                               const components::ComponentContext& context)
    : HttpHandlerBase(config, context),
      store_(context.FindComponent<components::Postgres>("postgres-db").GetCluster()) {}
