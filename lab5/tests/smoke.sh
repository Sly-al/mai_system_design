...
wait_http "$U/health" "user-service"
wait_http "$H/health" "hotel-service"
wait_http "$B/health" "booking-service"

curl -sf "$U/health" | grep -q '"service":"user"'
curl -sf "$H/health" | grep -q '"service":"hotel"'
curl -sf "$B/health" | grep -q '"service":"booking"'

curl -sf "$H/v1/hotels" | grep -q 'Grand Moscow'
RL_HEADERS=$(curl -sI "$H/v1/hotels/by-city?city=Moscow")
echo "$RL_HEADERS" | grep -qi 'X-RateLimit-Limit:'
echo "$RL_HEADERS" | grep -qi 'X-RateLimit-Remaining:'
echo "$RL_HEADERS" | grep -qi 'X-RateLimit-Reset:'
curl -sf "$H/v1/internal/hotels/1" | grep -q 'Tverskaya'

LOGIN="alice_$(date +%s)$$"
curl -sf -X POST "$U/v1/users" \
  -H 'Content-Type: application/json' \
  -d "{\"login\":\"$LOGIN\",\"password\":\"secret\",\"first_name\":\"Ali\",\"last_name\":\"Ce\"}" | grep -q '"id"'
