...
HotelCreateHandler::HotelCreateHandler(const components::ComponentConfig& config,
                                       const components::ComponentContext& context)
    : HttpHandlerBase(config, context),
      store_(context.FindComponent<components::Mongo>("mongo-db").GetPool()) {}
...
std::string HotelCreateHandler::HandleRequest(server::http::HttpRequest& request,
  ...
  const auto h = store_.Create(*principal, std::move(in.name), std::move(in.city), std::move(in.address));
  HotelListCache().InvalidateForCity(h.city);
  request.GetHttpResponse().SetStatus(server::http::HttpStatus::kCreated);
  return HotelJsonString(h);
}

HotelListHandler::HotelListHandler(const components::ComponentConfig& config,
                                   const components::ComponentContext& context)
    : HttpHandlerBase(config, context),
      store_(context.FindComponent<components::Mongo>("mongo-db").GetPool()) {}
...
std::string HotelListHandler::HandleRequest(server::http::HttpRequest& request,
  ...
  hb::SetJsonContentType(request);
  auto& cache = HotelListCache();
  const std::string key = ResponseCache::kListKey;
  if (const auto cached = cache.Get(key)) {
    return *cached;
  }
  const auto body = HotelsListJson(store_.ListAll());
  cache.Put(key, body);
  return body;
}

HotelByCityHandler::HotelByCityHandler(const components::ComponentConfig& config,
                                       const components::ComponentContext& context)
    : HttpHandlerBase(config, context),
      store_(context.FindComponent<components::Mongo>("mongo-db").GetPool()) {}
...
std::string HotelByCityHandler::HandleRequest(server::http::HttpRequest& request,
  ...
  const auto rl_key = std::string{"by_city:"} + ClientIp(request);
  auto decision = ByCitySearchLimiter().Admit(rl_key);
  ApplyRateLimitHeaders(request, decision);
  if (!decision.allowed) {
    request.GetHttpResponse().SetStatus(server::http::HttpStatus::kTooManyRequests);
    return hb::ErrorJson(hb::errors::kTooManyRequests);
  }

  auto& cache = HotelListCache();
  const auto ckey = ResponseCache::CityKey(city);
  if (const auto cached = cache.Get(ckey)) {
    return *cached;
  }
  const auto body = HotelsListJson(store_.ByCity(city));
  cache.Put(ckey, body);
  return body;
}

HotelInternalGetHandler::HotelInternalGetHandler(const components::ComponentConfig& config,
                                                 const components::ComponentContext& context)
    : HttpHandlerBase(config, context),
      store_(context.FindComponent<components::Mongo>("mongo-db").GetPool()) {}
